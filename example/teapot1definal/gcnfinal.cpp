...
void calculateEdgeValAndIndex() {
    ...
    for (int i = 0; i < raw_graph.size(); i += 2) {
        int source = raw_graph[i];
        int end = raw_graph[i + 1];

        edge_index[end].push_back(source); 
        degree[source]++;
    }
    ...
    for (int i = 0; i < v_num; i++) {
        for (int j = 0; j < edge_index[i].size(); j++) {
            float val = 1.0f / sqrt(degree[i] * degree[edge_index[i][j]]);
